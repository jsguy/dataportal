/*
	...
	* Serve Pub/Sub for many clients
	* Allow implementer to intercept and shape how changes are applied
	* Allow client to create adaptors for projects such as knockout.js models, etc...
...
module.exports = function(args){
	...
	var dataPortal = sockjs.createServer(args.server),
		...
		//	Create a clean response, with allowed data/types.
		//	TODO: define a standard for this, or use something like
		//	maybe JSON RPC 2.0 style? - it MUST be lightweight - 
		//	also we shouldn't trust the data, so do some cleanup here.
		createResponse = function(type, topic, data, cId){
			var response = {
				type: type,
				topic: topic
			};
		    ...
		    if(args.createResponse) {
		    	response = args.createResponse(response, cId);
		    }
		    return response;
		},
			...
			for (var key in subscriptions) {
				for(var j = 0; j < subscriptions[key].length; j += 1) {
					...
					if(cId !== subscriptions[key][j] && connections[subscriptions[key][j]]) {
						sendMessage(connections[subscriptions[key][j]], createResponse("diff", topic, message, cId));
					}
				}
			}
		},
